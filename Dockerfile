...
# Install main Python dependencies from requirements.txt using pip.
...
# IMPORTANT: Ensure 'uv' (and other pip-installed binaries) are in the PATH.
# In slim images, pip often installs executables to /root/.local/bin when run as root.
...
# This includes everything installed via pip and uv.
COPY --from=builder /usr/local/lib/python3.11/site-packages /usr/local/lib/python3.11/site-packages

# Copy the 'uv' and 'playwright' CLI executables from the builder stage.
# They are typically installed by pip/uv into /root/.local/bin.
COPY --from=builder /root/.local/bin/uv /usr/local/bin/uv
COPY --from=builder /root/.local/bin/playwright /usr/local/bin/playwright

# --- THIS IS THE MOST CRITICAL STEP FOR YOUR PLAYWRIGHT BROWSER ERROR ---
